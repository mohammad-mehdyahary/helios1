...
class ProductController extends Controller
{
    ...
    public function UPWP(Request $request,$id)
    {
        $data = array();
    $data['product_name'] = $request->product_name;
    $data['product_code'] = $request->product_code;
    ...
    $data['buyone_getone'] = $request->buyone_getone;
        ...
        if ($update) {
            ...
            $notification = array(
                'message' => 'محصول با موفقیت بروزرسانی نشد',
                'alert-type' => 'error'
                 );
                 return Redirect()->route('all.product')->with($notification);
        }
    ...
    public function UPP(Request $request,$id)
    {
        ...
        // DB::table('brands')->insert($data);
           if ($image_one) {  
               ...
               $notification = array(
                    'message' => 'تصویر اول با موفقیت بروزرسانی شد',
                    'alert-type' => 'success'
                     );
                   return Redirect()->route('all.product')->with($notification);
           } 
           if ($image_two) {  
            ...
            $productimg = DB::table('products')->where('id',$id)->update($data);
            $notification = array(
                 'message' => 'تصویر دوم با موفقیت بروزرسانی شد',
                 'alert-type' => 'success'
                  );
                return Redirect()->route('all.product')->with($notification);
        } 
        if ($image_three) {  
            ...
            $productimg = DB::table('products')->where('id',$id)->update($data);
            $notification = array(
                 'message' => 'تصویر سوم با موفقیت بروزرسانی شد',
                 'alert-type' => 'success'
                  );
                return Redirect()->route('all.product')->with($notification);
        } 
